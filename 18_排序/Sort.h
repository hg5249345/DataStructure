...
void Sort<Type>::QuickSort(DataList<Type> &list, int left, int right) {
    if (-3 == right){
    ...
    if (left < right){
        ...
        for (int i=left+1; i<=right; i++){
			//cout << "m_pvector[" << i << "] : " << list.m_pvector[i].GetKey() << ", pivotpos : " << pivotpos << ", pivot : " << pivot.GetKey() << endl; 
		...
		//cout << "m_pvector[" << left << "] : " << list.m_pvector[left].GetKey() << ", m_pvector[" << pivotpos << "] : " << list.m_pvector[pivotpos].GetKey() << endl;
...
{
	int i = 0;
    for (i=(list.m_ncurrentsize-2)/2; i>=0; i--){
        HeapAdjust(list, i, list.m_ncurrentsize-1);
    }

    for (i=list.m_ncurrentsize-1; i>=1; i--){
...
void Sort<Type>::RadixSort(DataList<int> &list, int m, int d)
{
    ...
    for (int i=0; i<m; i++){
		...
		int j = 0;
        for (j=0; j<list.m_ncurrentsize; j++){
