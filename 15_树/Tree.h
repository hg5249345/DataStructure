...
void Tree<Type>::Remove(TreeNode<Type> *current)
{
    if(NULL == current){
        return;
    }
...
template<typename Type> 
void Tree<Type>::Remove(Type item)
{
...
template<typename Type> 
TreeNode<Type>* Tree<Type>::Parent
(
...
template<typename Type> 
void Tree<Type>::Print(TreeNode<Type> *start, int n){
    if (NULL == start){
        for (int i=0; i<n; i++){
            cout << "     ";
        }
