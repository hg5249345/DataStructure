...
private:
    ...
    int m_nnumvertex;
    const int m_nMaxNum;
    static const int m_nDefaultSize;       //the default maximize vertex
    static const DistType m_Infinity;  //there is no edge
    int m_nnumedges;
    int Getvertexpos(const NameType vertex);    //get the vertex's position with the name of vertex
};

template<typename NameType, typename DistType>
const int Graph<NameType, DistType>::m_nDefaultSize = 10;

template<typename NameType, typename DistType>
const DistType Graph<NameType, DistType>::m_Infinity = 100000;
...
template<typename NameType, typename DistType> bool Graph<NameType, DistType>::Removevertex(int v){
    ...
    this->m_nnumvertex--;
    for (i=v; i<this->m_nnumvertex; i++)    //delete the vertex
    {
