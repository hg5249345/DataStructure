...
class SingleList{
public:
	SingleList() : head(new ListNode<Type>()), m_singleListLen(0) {}
	~SingleList(){
		MakeEmpty();
		delete head;
	}
...
*/
	...
	return m_singleListLen;
}

template<typename Type> 
ListNode<Type>* SingleList<Type>::Find(int n)
{
	if (n < 0 || n >= m_singleListLen)
	{
...
ListNode<Type>* SingleList<Type>::Find(Type value, int n){
	...
	while(count != n && pmove){
		pmove=pmove->m_pnext;
		if(NULL != pmove && pmove->m_data==value){
			count++;
		}
	}
